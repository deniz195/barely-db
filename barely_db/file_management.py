...
class RevisionFile(object):
    ...
    def get_next_revision_file(self):
        ...
        return RevisionFile(base_name=self.base_name, revision=current_revision + 1,)
    ...
    def get_new_revision(self):
        ...
        if self.revision is None:
            ...
            rf = RevisionFile(base_name=new_name, revision=0)
        else:
            fn_name = Path(self.base_name).name
            new_name = path_to_old_rev.joinpath(fn_name)
            rf = RevisionFile(base_name=new_name, revision=self.revision)
...
def serialize_to_file(
    ...
    file_serializer = ClassFileSerializer(
        base_file_identifier=base_file_identifier,
    ...
    def decorate_class(cls):
        ...
        cls.get_serialization_filename = functools.partialmethod(
            file_serializer.get_serialization_filename_from_entity
        )
